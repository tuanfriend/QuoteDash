...
class UserManager(models.Manager):
    ...
    def quote_validator(self, postData):
        errors = {}
        # check quote content
        if len(postData['auth-form']) == 0:
            errors['auth-form'] = 'Please enter author name.'
        elif len(postData['auth-form']) < 3:
            errors["auth-form"] = "Author name should be at least 2 characters"
        #check quote
        if len(postData['quo-form']) == 0:
            errors['quo-form'] = 'Quote cannot leave blank.'
        elif len(postData['quo-form']) < 10:
            errors["quo-form"] = "Quote should be at least 10 characters"
        return errors
...
class Quotes(models.Model):
    user = models.ForeignKey(User, related_name="rela_user")
    auth = models.CharField(max_length=255)
    quote = models.TextField()
    like = models.ManyToManyField(User, related_name="liked_quote")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    objects = UserManager()